/*
Example code showing how to perform oriented bounding box object detection
...
func main() {
	...
	yoloProcesser := postprocess.NewYOLOv8obb(postprocess.YOLOv8obbDOTAv1Params())
	...
	for _, detResult := range detectResults {
		fmt.Printf("%s @ (%d %d %d %d angle=%f) %f\n", classNames[detResult.Class], detResult.Box.X, detResult.Box.Y, detResult.Box.Width, detResult.Box.Height, detResult.Box.Angle, detResult.Probability)
	}
