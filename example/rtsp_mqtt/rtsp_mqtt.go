...
type Demo struct {
	...
	// mqttBroker MQTT broker地址
	mqttBroker string
	// mqttTopic MQTT主题
	mqttTopic string
	// alertThreshold 告警阈值（检测到目标数量超过此值时触发告警）
	alertThreshold int
	// alertCooldown 告警冷却时间（秒）
	alertCooldown int
	// lastAlertTime 上次告警时间
	lastAlertTime time.Time
	// alertMutex 用于保护告警相关字段的互斥锁
...
func NewDemo(rtspSrc *RTSPSource, modelFile, labelFile string, poolSize int,
	...
	d := &Demo{
		...
		alertThreshold: 1,  // 默认告警阈值为1
		alertCooldown:  30, // 默认告警冷却时间为30秒
	}
...
func (d *Demo) ProcessFrame(img gocv.Mat, retChan chan<- ResultFrame,
	...
	if d.mqttClient != nil && d.mqttClient.IsConnected() {
		d.AlertOut(detectResults, trackObjs, segMask)
	}
...
// AlertOut 处理告警并发送MQTT消息
func (d *Demo) AlertOut(detectResults []postprocess.DetectResult, trackObjs []*tracker.STrack, segMask postprocess.SegMask) {
	d.alertMutex.Lock()
	defer d.alertMutex.Unlock()

	// 检查是否在冷却期内
	if time.Since(d.lastAlertTime).Seconds() < float64(d.alertCooldown) {
		return
	}

	// 统计检测到的目标数量
	objCount := len(trackObjs)
	if objCount < d.alertThreshold {
		return
	}
	...
	// 收集检测到的对象类别
	for _, track := range trackObjs {
		if track.GetLabel() < len(d.labels) {
			alertMsg.Objects = append(alertMsg.Objects, d.labels[track.GetLabel()])
		}
	}
	...
	if err != nil {
		log.Printf("Error marshaling alert message: %v", err)
		return
	}

	// 发送MQTT消息
	if token := d.mqttClient.Publish(d.mqttTopic, 0, false, jsonData); token.Wait() && token.Error() != nil {
		log.Printf("Error publishing alert message: %v", token.Error())
		return
	}

	// 更新最后告警时间
	d.lastAlertTime = time.Now()
	log.Printf("Alert sent: %d objects detected", objCount)
}
...
func main() {
	...
	*codec = strings.ToLower(*codec)
	...
	// 设置告警参数
	demo.alertThreshold = *alertThreshold
	demo.alertCooldown = *alertCooldown
